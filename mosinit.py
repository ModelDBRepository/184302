'''
Current clamp demo using Allen Brain's model 472450023.
...
from neuron472450023 import Neuron472450023
...
def demo(iapp):
    ...
    h.celsius = 34.0
    cell = Neuron472450023(name='neuron')
    ...
    vbox.intercept(0)
    vbox.map('Allen Institute model 472450023; v includes junction potential', 10, 10, 600, 400)

    # procedure for doing and plotting each simulation
    def do_current_clamp_experiment(amp, color):   
        ic.amp = amp / 1000.
        
        h.finitialize(-80.9754867554)
        h.fcurrent()
